...
def wheelIncome(fortune_wheel_data):
    ...
    prize = random.choices(labels, weights=weights, k=1)[
        0
    ]  # random value based on weights
    ...
    degree = round(
        random.uniform(degree_start + 0.1, degree_end - 0.1), 2
    )  # random degree the is in certain sector
