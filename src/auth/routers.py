...
async def db_upload_photo(
    session: AsyncSession, currUser: User, file: UploadFile = File(...)
):
    try:
        ...
        if currUser.photo and currUser.photo != "/defaultUserPic.png":
            try:
                old_file_path = os.path.join(
                    "src/imgs/users_avatars", currUser.photo.lstrip("/")
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"failed uploading photo (games/crud): {e}"
        )
