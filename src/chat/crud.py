...
from src.chat.models import Message, User, MessageSchema
from datetime import datetime

async def db_save_message(data_message, user_id: int, session: AsyncSession = None) -> None:
    try:
        timestamp = datetime.fromisoformat(data_message["timestamp"].replace("Z", "+00:00"))
        stmt = insert(Message).values(user_id=user_id, message=data_message["message"], timestamp=timestamp)
